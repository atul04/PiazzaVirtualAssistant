...
# @Last modified time: 2019-09-23T23:29:11+05:30
...
def cosine_similarity_custom(spacy_model, query, document):
...
def cosine_query_similarity(doc_path, query_path):
    ...
    for ind in range(df_size):
        ...
        h_sim = cosine_similarity_custom(nlp_e, df_query.iloc[0]['TITLE'], df.iloc[ind]['TITLE'])
        c_sim = cosine_similarity_custom(nlp_e, df_query.iloc[0]['MAIN_CONTENT'], df.iloc[ind]['MAIN_CONTENT'])
...
#         c_sim = cosine_similarity(nlp_e, df_query.iloc[0]['MAIN_CONTENT'], df.iloc[ind]['MAIN_CONTENT'])
        q_w = headc_w*h_sim + contc_w*c_sim
        cos_sim.append(q_w)
        # h_title.append(h_sim)
        # c_content.append(c_sim)

    # import operator
    # index, value = max(enumerate(cos_sim), key=operator.itemgetter(1))

    cos_sim = np.asarray(cos_sim)
    # h_title = np.argsort(h_title)
    # c_content = np.argsort(c_content)
    # indx_h = h_title.argsort()[-50:][::-1]
    # indx_c = c_content.argsort()[-50:][::-1]
    indx = cos_sim.argsort()[-5:][::-1]
    ...
    print(" ID               SUMMARY                      QUERY                            "
          "    THEME                   SIMILARITY")
    # intersection_set = []
    # for i in range(len(indx_h)):
    #     for j in range(len(indx_c)):
    #         print(indx_h[i],indx_c[j])
    #         if(indx_h[i] == indx_c[j]):
    #             intersection_set.append([df.loc[[i]].to_string(header=False, index=False), h_title[i], c_content[j]])
...
def cosine_query__bert_int_similarity(doc_path, query_path):
    df_query = pd.read_csv(query_path,
                           sep='\t',
                           header=0
                           )
...
#         c_sim = cosine_similarity(nlp_e, df_query.iloc[0]['MAIN_CONTENT'], df.iloc[ind]['MAIN_CONTENT'])
    ...
    print("Top Similar queries:\n")
    print(" ID               SUMMARY                      QUERY                            "
          "    THEME                   H_SIMILARITY                   C_SIMILARITY")
    intersection_set = []
    for i in range(len(indx_h)):
        for j in range(len(indx_c)):
            print(indx_h[i],indx_c[j])
...
#
#
# def cosine_query__bert_soft_similarity(doc_path, query_path):
#     df_query = pd.read_csv(query_path,
#                            sep='\t',
#                            header=0
#                            )
...
#
# #         print(c, d)
#         h_sim = softcossim(bert_sent_emd(df_query.iloc[0]['TITLE']).reshape(1,-1), bert_sent_emd(df.iloc[ind]['TITLE']).reshape(1,-1), similarity_matrix)[0][0]
# #         print(h_sim)
# #         h_sim = cosine_similarity(nlp_e,df_query.iloc[0]['TITLE'], df.iloc[ind]['TITLE'])
#         c_sim = softcossim(bert_sent_emd(df_query.iloc[0]['MAIN_CONTENT']).reshape(1,-1), bert_sent_emd(df.iloc[ind]['MAIN_CONTENT'], similarity_matrix).reshape(1,-1))[0][0]
# #         c_sim = cosine_similarity(nlp_e, df_query.iloc[0]['MAIN_CONTENT'], df.iloc[ind]['MAIN_CONTENT'])
#         q_w = headc_w*h_sim + contc_w*c_sim
#
#         cos_sim.append(q_w)
...
def cosine_query__bert_2layer_similarity(doc_path, query_path):
    df_query = pd.read_csv(query_path,
                           sep='\t',
                           header=0
                           )
...
#         c = bert_sent_emd(df_query.iloc[0]['TITLE']).shape
#         d = bert_sent_emd(df_query.iloc[ind]['TITLE']).shape
        if(cosine_similarity(bert_sent_emd(df.iloc[ind]['TITLE']).reshape(1,-1), bert_sent_emd(df.iloc[ind]['MAIN_CONTENT']).reshape(1,-1))[0][0] < 0.45):
            continue
...
#         c_sim = cosine_similarity(nlp_e, df_query.iloc[0]['MAIN_CONTENT'], df.iloc[ind]['MAIN_CONTENT'])
    ...
    print("#######################################################################################################")
    print("COSINE SIMILARITY BERT 2 layer")
    print("#######################################################################################################")
    print("Top Similar queries:\n")
    print(" ID               SUMMARY                      QUERY                            "
          "    THEME                   SIMILARITY")
...
def cosine_query__bert_cross_similarity(doc_path, query_path):
    df_query = pd.read_csv(query_path,
                           sep='\t',
                           header=0
                           )
    ...
    titlec_w = 0.9
    headc_w = 0.02
    contc_w = 0.48 #1 - headc_w -titlec_w
...
#         print(c, d)
        t_q = bert_sent_emd(df_query.iloc[0]['TITLE']).reshape(1,-1)
        t_c = bert_sent_emd(df.iloc[ind]['TITLE']).reshape(1,-1)
        c_q = bert_sent_emd(df_query.iloc[0]['MAIN_CONTENT'][:511]).reshape(1,-1)
        c_c = bert_sent_emd(df.iloc[ind]['MAIN_CONTENT'][:511]).reshape(1,-1)
        t_sim = cosine_similarity(t_q,t_c )[0][0]
        h_sim = cosine_similarity(t_q, c_c)[0][0]
#         print(h_sim)
#         h_sim = cosine_similarity(nlp_e,df_query.iloc[0]['TITLE'], df.iloc[ind]['TITLE'])
        c_sim = cosine_similarity(c_q, c_c)[0][0]
#         c_sim = cosine_similarity(nlp_e, df_query.iloc[0]['MAIN_CONTENT'], df.iloc[ind]['MAIN_CONTENT'])
        q_w = (headc_w*h_sim + contc_w*c_sim + titlec_w*t_sim)/(headc_w+contc_w+titlec_w)
        if(q_w >= 0.7):
            cos_sim.append(q_w)
        # h_title.append(h_sim)
        # c_content.append(c_sim)
...
if __name__== "__main__":
    ...
    # print()
    #
    # cosine_query__bert_2layer_similarity("dataset/PIAZZA_clean_single_theme.tsv", "query.tsv")
    ...
    print()
    # start = time.time()
    # cosine_query__bert_cross_similarity("dataset/PIAZZA_clean_single_theme.tsv", "query.tsv")
    # ###############################\n")
    # end = time.time()
    # print("time taken in sec: "+str(end-start))
    # print("###########################################\n")
